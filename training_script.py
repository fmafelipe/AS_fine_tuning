...
mi_dataset_df =pd.read_csv("corpus1_balanceado_3labels.csv", sep=";")
...
features = Features({"text": Value("string"), "label": ClassLabel(num_classes=3, names=["negativo","neutro","positivo"])})
...
model = AutoModelForSequenceClassification.from_pretrained("bert-base-multilingual-uncased", num_labels=3).to(device)
